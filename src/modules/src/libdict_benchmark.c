/* A benchmark for the libdict framework, adapted for the crazyflie from https://github.com/fmela/libdict/blob/4e0673bcc6b0308c266d239674e99daabe7eeb87/benchmark.c
 */
...
void execute_benchmark(const char algorithmSelection) {
  srand(0xdeadbeef);

  dict_malloc_func = xmalloc;

  const char type = algorithmSelection;
  ...
  size_t words_read = 0;
  while (words_read < nwords) {
    char str[5];
    sprintf(str, "%d", 1000 + words_read);
    words[words_read++] = str;
  }
